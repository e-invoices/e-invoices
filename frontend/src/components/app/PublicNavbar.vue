  ...
  <header class="bg-white dark:bg-slate-900 shadow-sm dark:shadow-slate-800">
    <div class="w-full px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16 md:h-20">
        ...
        <nav class="hidden md:flex items-center justify-center gap-6 lg:gap-8">
          <RouterLink
            v-for="item in menuItems"
            :key="item.path"
            :to="item.path"
            class="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-medium transition-colors duration-200 no-underline whitespace-nowrap"
          >
            {{ item.label }}
          </RouterLink>
        </nav>
        ...
        <div class="hidden md:flex items-center justify-end gap-2 flex-1">
          <!-- Theme Toggle -->
          <ThemeToggle />
          ...
          <div v-if="!isAuthenticated" class="flex items-center gap-3 ml-3">
            <RouterLink
              to="/login"
              class="px-4 lg:px-6 py-2 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white font-semibold rounded-full hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-200 bg-transparent cursor-pointer text-sm whitespace-nowrap no-underline"
            >
              {{ t('nav.login') }}
            </RouterLink>
            <RouterLink
              to="/register"
              class="px-4 lg:px-6 py-2 bg-blue-500 text-white font-semibold rounded-full hover:bg-blue-600 transition-all duration-200 border-none cursor-pointer text-sm whitespace-nowrap no-underline"
            >
              {{ t('nav.register') }}
            </RouterLink>
          </div>
        ...
        <div class="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-3">
          ...
          <div v-if="!isAuthenticated" class="flex flex-col gap-2 px-3">
            <RouterLink
              ...
              class="w-full text-center px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white font-semibold rounded-full hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-200 bg-transparent cursor-pointer text-sm no-underline"
            >
              {{ t('nav.login') }}
            </RouterLink>
            <RouterLink
              to="/register"
              @click="closeMobileMenu"
              class="w-full text-center px-4 py-2 bg-blue-500 text-white font-semibold rounded-full hover:bg-blue-600 transition-all duration-200 border-none cursor-pointer text-sm no-underline"
            >
              {{ t('nav.register') }}
            </RouterLink>
          </div>
